...
async function callServer(target) {
  ...
  let filename = "C:\\Users\\AbulHasan\\Videos\\MOVIES\\Hollywood\\Orphan.mp4";

  // let filename ="C:\\Users\\AbulHasan\\Downloads\\archive\\crime-data-from-2010-to-present.csv";
  // let filename = "C:\\Users\\AbulHasan\\Documents\\Development\\repos\\backend\\grpc\\examples\\node\\A.json";
  ...
  let responseRec = true;
  client.gotFile({ index: count, payload: file }, (err, response) => {
    console.log("Greeting:", response);
  // responseRec = true;
  // client.close();
});
  // while (byteStart < file.length) {
  //   if (count === 0 || responseRec) {
  //     // console.log('hey')
  //     count += 1;
  ...
  client.gotFile(
    { index: -1, totalsize: 1, filename: filename1 },
    (err, response) => {
